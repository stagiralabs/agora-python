...
async def main() -> None:
    async with AsyncAgoraClient(BASE_URL) as client:
        try:
            org_ids = await client.market.list_organization_ids()
            print("Organization ids:", org_ids)

            agents = await client.market.list_all_agents()
            print(f"Agents returned: {len(agents)}")

            wallets = await client.market.list_all_wallets()
            print(f"Wallets returned: {len(wallets)}")

            offers = await client.market.list_offers()
            print(f"Offers returned: {len(offers)}")

            target_statuses = await client.market.get_all_target_statuses()
        ...
        if ORGANIZATION_ID:
            org_wallets = await client.market.list_organization_wallets(ORGANIZATION_ID)
            print(f"Wallets for org {ORGANIZATION_ID}: {len(org_wallets)}")

        if TARGET_IDS:
            specific_statuses = await client.market.get_specific_target_statuses(TARGET_IDS)
